...
void get_initial_frame_field(const Args &args, MeshT &tetmesh)
{
  ...
  if (!args.inFileName.empty()) {
    ...
    if (!args.rfOutFileName.empty()) {
      ...
      write_ovm_file(args.rfOutFileName, tetmesh);
  ...
  else if (!args.rfInFileName.empty()) { //read singular graph, field and tetmesh from data
    //read ovmb file
...
void write_ovmb_file(const std::string &filename, MeshT &mesh)
{
  ...
  template<typename MeshT>
void write_ovm_file(const std::string &filename, MeshT &mesh)
{
